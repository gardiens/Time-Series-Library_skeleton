...
class time_serie_NTU_2:
    ...
    def get_data(self,row):
        ...
        debut_frame=int(row['debut_frame'])
        #* On récupère le début et la fin de la séquence
        #*Ici particularité: On va raccourcir le temps
        debut=debut_frame
        data=data[debut:] #*on regarde queà partir du problème

        n=len(data)
        k=n//self.output_len
        
        conv=np.ones(k)/k
        data=np.apply_along_axis(lambda m:np.convolve(m,conv,mode='valid'),axis=0,arr=data)[::k][:self.output_len,:,:]# On veut que la moyenne

        
        begin=data[:self.input_len]
        label=data[:self.output_len]#* On prend les output_len suivantes
...
class time_serie_NTU_FED1:
    ...
    def get_data(self,row):
        ...
        begin=begin.reshape(begin.shape[0],-1)
        label=label.reshape(label.shape[0],-1)
        begin=np.fft.fft(begin,axis=0)
        label=np.fft.fft(label,axis=0)
        #* Maintenant , begin est de la forme( nb_frames,nb_joints*3) et label est de la forme (nb_frames,nb_joints*3
...
class time_serie_NTU_FEDn:
    ...
    def get_data(self,row):
        ...
        begin=begin.reshape(begin.shape[0],-1)
        label=label.reshape(label.shape[0],-1)
        begin=np.fft.fftn(begin)
        label=np.fft.fftn(label)
        #* Maintenant , begin est de la forme( nb_frames,nb_joints*3) et label est de la forme (nb_frames,nb_joints*3
